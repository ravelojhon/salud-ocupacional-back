...
@Injectable()
export class FactorRiesgoQuimicoService {
    async getFactorRiesgoQuimico() {
        try {
            ...
            const resultado = await result.execute('sp_factor_riesgo_quimico');
            return {
            ...
            console.log(error);
            return {
                mensaje: 'Error al listar factores de riesgo químico',
